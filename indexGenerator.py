...
# Define the folder containing images and output file
image_folder = "dist/assets/portfolio"
output_file = "index.html"

# Get a list of image files
image_extensions = (".jpg", ".jpeg", ".png", ".webp")
images = sorted([img for img in os.listdir(image_folder) if img.lower().endswith(image_extensions)])
...
<head>
  ...
  <title>Puzzles Studios</title>
  <meta name="description"
    content="Hello there, I'm Mamzi Osouli – an intrepid explorer through the lens, on a relentless quest to capture the world's boundless wonders. Join me in unraveling the extraordinary through my lens as I embark on a visual journey like no other.">
  <!-- Favicon -->
    ...
    <nav id="nav" x-data="{{ open: false }}" role="navigation" class="w-full">
      <div class="container mx-auto flex flex-wrap items-center md:flex-no-wrap">
        <div class="mr-4 md:mr-8">
          <a href="index.html" class="text-2xl font-signika font-bold">PUZZLES STUDIOS</a>
        </div>
  ...
  <footer>
    <div class="max-w-screen-xl py-16 mx-auto">
      <div class="grid grid-cols-1 gap-8 text-center mx-auto">
        <div>
          <p class="font-signika"><b>Mamzi Osouli</b></p>
          <p class="mt-4 text-sm text-gray-600 dark:text-gray-300">
            77 Spruce pl sw Calgary AB<br />(403) 796-4691
          </p>
          <div class="flex mx-auto">
            <div class="mx-auto space-x-6 flex mt-8 text-gray-600 dark:text-gray-300">
              ...
              <a class="transition duration-300 hover:opacity-75" href="https://www.behance.net" target="_blank"
                rel="noreferrer">
...
for idx, img in enumerate(images):
    # Determine if we should use a different layout for the image
    image = Image.open(os.path.join(image_folder, img))
    orientation_class = "md:w-1/4" if image.size[0] < image.size[1] else "md:w-1/2"
    column_class = "p-1"
    images_html += f'''
    <div class="w-full { orientation_class } {column_class}">
        <div class="overflow-hidden h-full">
            <a href="{image_folder}/{img}" data-fancybox="gallery">
                <img alt="Image" 
                      class="block w-full h-auto object-cover object-center opacity-0 animate-fade-in transition duration-500 transform scale-100 hover:scale-110" 
                      src="{image_folder}/{img}" />
            </a>
        </div>
    </div>
    '''
...
with open(output_file, "w", encoding="utf-8") as f:
    f.write(html_template.format(images_html=images_html))
